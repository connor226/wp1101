...
export default (boardSize, mineNum) => {
    ...
    mineLocations.map(mine => {
        for(let i = -1; i <= 1; i++){
            for(let j = -1; j <= 1; j++){
                if(mine[0] + i < 0 || mine[0] + i >= boardSize)  continue;
                else if(mine[1] + j < 0 || mine[1] + j >= boardSize)  continue;
                else if(typeof(board[mine[0] + i][mine[1] + j].value) === 'number'){
                    board[mine[0]+i][mine[1]+j].value += 1;
                }
            }
        }
    })
